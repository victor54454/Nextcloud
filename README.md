...
```bash
docker exec nextcloud-ldap-cert-nextcloud-1 su -s /bin/sh www-data -c "php occ app:install onlyoffice"
```

### 3. Configurer l'URL du serveur OnlyOffice
```bash
docker exec nextcloud-ldap-cert-nextcloud-1 su -s /bin/sh www-data -c "php occ config:app:set onlyoffice DocumentServerUrl --value='http://documentserver/'"
```

### 4. Configurer le secret JWT
```bash
docker exec nextcloud-ldap-cert-nextcloud-1 su -s /bin/sh www-data -c "php occ config:app:set onlyoffice jwt_secret --value='super-secret-jwt-2024-change-me'"
```
...
```bash
docker exec nextcloud-ldap-cert-nextcloud-1 su -s /bin/sh www-data -c "php occ config:app:set onlyoffice defFormats --value='{\"docx\":true,\"xlsx\":true,\"pptx\":true}'"
```
### 6. Activer Draw.io 
```bash 
docker exec nextcloud-ldap-cert-nextcloud-1 su -s /bin/sh www-data -c "php occ app:install drawio"
```
...
```bash
docker logs nextcloud-ldap-cert-documentserver-1
```